...
import { z } from 'zod';
import { Clock, MapPin, CalendarRange, Route as RouteIcon, Bus } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { fetchBuses, fetchRoutes, fetchRoute } from '@/api';
import { fetchSchedule } from '@/api';
import { updateSchedule } from '@/api';
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select";
import Navbar from '@/components/Nabvar';
import { Bus as BusType, Route, ScheduleItem } from '@/types';
...
const scheduleSchema = z.object({
    ...
    repeatDays: z.array(z.enum(['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'])).optional(),
    isOneTime: z.boolean(),
});
...
export default function UpdateSchedule() {
    ...
    useEffect(() => {
                ...
                if (id) {
                    ...
                    if (schedule) {
                        ...
                        const stopsWithTime = schedule.stops.map((stop: {
                            ...
                            stopId: stop.stopId,
                            startTime: new Date(stop.startTime).toISOString().substring(11, 16),
                            endTime: new Date(stop.endTime).toISOString().substring(11, 16),
                        }));
                        ...
                        reset({
                            ...
                            repeatDays: (schedule.repeatDays || []).filter(
                                (day): day is 'Sun' | 'Mon' | 'Tue' | 'Wed' | 'Thu' | 'Fri' | 'Sat' =>
                                    ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].includes(day)
                            ),
                            stops: stopsWithTime,
                        });
                        setSelectedBusId(schedule.busId);
        ...
        loadData();
    }, [id, dispatch, reset, setValue]);

    useEffect(() => {
        const fetchAndSetRoute = async () => {
            if (selectedRouteId) {
                try {
                    const route = await fetchRoute(selectedRouteId);
    ...
    const onSubmit = async (data: ScheduleFormData) => {
        ...
        try {
            ...
            const stopsWithISO = data.stops.map((s) => ({
                ...s,
                startTime: new Date(`1970-01-01T${s.startTime}:00Z`).toISOString(),
                endTime: new Date(`1970-01-01T${s.endTime}:00Z`).toISOString(),
            }));
            ...
            const finalPayload = {
                ...
                repeatDays: isOneTime ? [] : data.repeatDays,
            };


            dispatch(setProgress(60));
            const result = await updateSchedule(id!, finalPayload);
            if (result.success) {
                dispatch(setProgress(100));
                navigate("/schedules");
